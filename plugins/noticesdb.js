...
module.exports = {
    ...
    updateById: function (notice, id, params, callback) {
        //每次使用的时候需要创建链接，数据操作完成之后要关闭连接
        let sql = "update notices set title='" + notice.title + "',detail=" + "'" + notice.detail + "',createtime=" + "'" + notice.createtime + "',updatetime=" + "'" + notice.updatetime + "',author=" + "'" + notice.author  + "',type=" + "'" + notice.type + "',cover=" + "'" + notice.cover + "' where id =" + id
        console.log("修改公告")
        console.log(sql)
        var connection = mysql.createConnection(data);
        connection.connect(function (err) {
